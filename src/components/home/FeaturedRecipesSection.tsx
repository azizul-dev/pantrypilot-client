      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-4">
          <div>
            ...
            <h2 className="font-poppins font-extrabold text-3xl sm:text-4xl text-secondary mt-1">
              Featured Recipes
            </h2>
          </div>
          <p className="text-text-brown/70 text-sm md:max-w-md">
            Check out these trending kitchen creations. Fresh, healthy, and
            easy-to-follow directions approved by top chefs.
          </p>
        </div>
        ...
        {isLoading ? (
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {Array.from({ length: 8 }).map((_, i) => (
              <div
                key={i}
                className="bg-white rounded-2xl p-6 shadow-sm border border-neutral-100 animate-pulse flex flex-col gap-4"
              >
          ...
          >
            {recipes.slice(0, 8).map((recipe) => (
              ...
              >
                <article className="pantry-card group cursor-pointer overflow-hidden flex flex-col justify-between h-full transition-all duration-500 ease-out hover:shadow-lg hover:-translate-y-1.5">
                  <div className="relative w-full h-32 sm:h-48 mb-3 sm:mb-4 rounded-xl overflow-hidden bg-neutral-100 shrink-0">
                    <img
                      src={
                        recipe.images?.[0] ||
                      ...
                      alt={recipe.title}
                      loading="eager"
                      className="absolute inset-0 w-full h-full object-cover transition-all duration-700 ease-in-out scale-100 group-hover:scale-110 group-hover:opacity-0"
                    />
                    {recipe.images?.[1] && (
                      <img
                        src={recipe.images[1]}
                        ...
                        loading="eager"
                        className="absolute inset-0 w-full h-full object-cover opacity-0 scale-110 transition-all duration-700 ease-in-out group-hover:opacity-100 group-hover:scale-100"
                      />
                    )}
                  ...
                  <div className="flex-1 flex flex-col gap-2">
                    ...
                    <p className="hidden sm:block text-text-brown/70 text-xs line-clamp-2 leading-relaxed">
                      {" "}
