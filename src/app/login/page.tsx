...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res?.error) {
        setErrors({ general: "Invalid email or password combination." });
      } else {
        router.push("/");
        router.refresh();
      }
  ...
  const handleDemoLogin = async () => {
    ...
    try {
      ...
      if (res?.error) {
        setErrors({ general: "Demo account login failed." });
      } else {
        router.push("/");
        router.refresh();
      }
    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-12 sm:px-6 lg:px-8 bg-bg-cream">
      ...
      >
        ...
        <div className="text-center flex flex-col items-center gap-2">
          ...
          <h2 className="font-poppins font-extrabold text-2xl text-secondary mt-2">Welcome Back</h2>
          <p className="text-xs text-text-brown/65">Sign in to manage your pantry and cook smart.</p>
        </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full bg-secondary hover:bg-secondary/95 text-white font-semibold py-3 rounded-xl text-sm flex items-center justify-center gap-2 transition-all shadow-xs"
          >
